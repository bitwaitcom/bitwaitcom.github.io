...
public class CoinService extends BaseService {
    ...
    public void setPlatform(Coin coin) {
        //取消其他平台币
        List<Coin> coins = coinDao.findAllByIsPlatformCoin(BooleanEnum.IS_TRUE);
        coins.forEach(x -> x.setIsPlatformCoin(BooleanEnum.IS_FALSE));
        //设置传入币为平台币
        coin.setIsPlatformCoin(BooleanEnum.IS_TRUE);
    }
