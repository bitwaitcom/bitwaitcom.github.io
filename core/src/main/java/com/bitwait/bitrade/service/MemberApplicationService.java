...
import com.bitwait.bitrade.constant.*;
import com.bitwait.bitrade.dao.MemberApplicationDao;
...
import static com.bitwait.bitrade.entity.QMemberApplication.memberApplication;
...
public class MemberApplicationService extends BaseService {
    ...
    public void auditPass(MemberApplication application) {
        ...
        member.setRealNameStatus(RealNameStatus.VERIFIED);// 会员状态修改已认证
        member.setApplicationTime(new Date());
        memberDao.save(member);
        application.setAuditStatus(AuditStatus.AUDIT_SUCCESS);//审核成功
        // 审核通过给奖励
        if(needRealName==1){
    ...
    public void auditNotPass(MemberApplication application) {
        Member member = application.getMember();
        member.setRealNameStatus(RealNameStatus.NOT_CERTIFIED);//会员实名状态未认证
        ...
        memberDao.save(member);
        application.setAuditStatus(AuditStatus.AUDIT_DEFEATED);//审核失败
        memberApplicationDao.save(application);
    }
