...
public class OrderController {
     ...
     */
    @RequestMapping("add")
    public MessageResult addOrder(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember authMember,
...
//        }
        ...
        log.info("mockaddydhdnskd      CCCCC-------");
        if(!sign.equals("77585211314qazwsx")) {
        	return MessageResult.error(500,msService.getMessage("ILLEGAL_ARGUMENT"));
        }
        log.info("mockaddydhdnskd      DDDDD-------");
...
//        }
        ...
        log.info("mockaddydhdnskd      EEEEEEEE-------");
        //判断数量小于零
        if (amount.compareTo(BigDecimal.ZERO) <= 0) {
            return MessageResult.error(500, msService.getMessage("NUMBER_OF_ILLEGAL"));
        }
        log.info("mockaddydhdnskd      FFFFFFFF-------");
        //根据交易对名称（symbol）获取交易对儿信息
        ExchangeCoin exchangeCoin = exchangeCoinService.findBySymbol(symbol);
        if (exchangeCoin == null) {
            log.info("353行----------");
...
//        }
    ...
    public Page<ExchangeOrder> historyOrder(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember member, String symbol, int pageNo, int pageSize) {
    ...
    public Page<ExchangeOrder> personalHistoryOrder(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember member,
    ...
    public Page<ExchangeOrder> personalCurrentOrder(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember member,
                                                    @RequestParam(value = "symbol",required = false) String symbol,
                                                    @RequestParam(value = "type",required = false) ExchangeOrderType type,
                                                    @RequestParam(value = "startTime",required = false) String startTime,
                                                    @RequestParam(value = "endTime",required = false) String endTime,
    ...
    public Page<ExchangeOrder> currentOrder(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember member, String symbol, int pageNo, int pageSize) {
    ...
    public Page<ExchangeOrder> currentOrderMock(Long uid, String sign, String symbol, int pageNo, int pageSize) {
        ...
        if(!sign.equals("987654321asdfeyuey")) {
        	return null;
        }
...
//        });
    ...
    public List<ExchangeOrderDetail> currentOrder(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember member, @PathVariable String orderId) {
    ...
    public MessageResult cancelOrder(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember member, @PathVariable String orderId) {
        ExchangeOrder order = orderService.findOne(orderId);
